...
# 단위 열거 런: "3인치/4인치/6인치/8인치"·"25mm/35mm"·"62mm(2인치)/83mm(3인치)" —
# 같은 단위(\1) 반복일 때만(괄호 병기 허용). 선행 한글 허용("전나무120cm/150cm"),
# 숫자/영문("1100x2000mm"의 x)은 차단.
_UNIT_ENUM_RE = re.compile(
    r"(?<![0-9A-Za-z./])\d+(?:\.\d+)?\s*(인치|cm|mm)(?:\([^()]{0,12}\))?"
    r"(?:\s*/\s*\d+(?:\.\d+)?\s*\1(?:\([^()]{0,12}\))?)+(?![\w가-힣])")
        ...
        for t in seg.split():
            if not out["watt"] and _WATT_OPT_RE.match(t):
                out["watt"] = _WATT_OPT_RE.match(t).group(1) + "W"
                                     ...
                                     or t in lex.CCT_SYNONYM
                                     or re.fullmatch(r"[1-9]\d{3}[Kk]", t)):
                out["cct"] = lex.CCT_SYNONYM.get(t, t.upper() if t[:1].isdigit() else t)
...
# 결제/은행 select 시그니처 — wooree 크롤 유입(231상품, 값 4,620) 실측. 변형 아님.
_BANK_GROUP_RE = re.compile(r"인터넷뱅킹|무통장|입금자|바로가기")


def _group_combos(grp_json, cap=400):
    """옵션군 JSON([{"label","values"}…]) → 군 간 교차 조합 [ {축:값, _delta:원}… ].

    군별: 값 정제(가격델타 분리 캡처) → _promote_option 으로 축 판정(값 우선) →
    못 알아낸 잔여는 라벨로 축 지정. 추가상품 군(라벨/선두값 신호)·은행 결제 군은 통째 제외.
    조합 수는 cap 상한(폭발 방지, 2026-07-03 100→400: cap 절단 95 PDP 실측).
    _delta 는 군 간 합산 — 변형가 = 상품가 + Σ델타."""
    ...
    for g in groups:
        ...
        raw_vals = g.get("values") or []
        if label == "_cascade":
            continue   # 캐스케이드 탐지 마커(수집 메타) — 변형 아님
        if _ADDON_GROUP_RE.search(label):
            continue   # 추가상품/부속 군 — 변형 아님
        if sum(1 for v in raw_vals if _BANK_GROUP_RE.search(v)
               or re.search(r"(은행|뱅크)\s*$", v)) >= 2:
            continue   # 결제안내 select — 변형 아님
        pairs = []
        for v in raw_vals:
            _, delta = _split_delta(re.sub(r"[★☆]+", " ", v))
            t = _clean_opt(v)
            if t and not _OPT_HEADER_RE.match(t):
                pairs.append((t, delta))
        if pairs and pairs[0][0] == "추가상품":
            continue
        seen_t = set()
        parsed = []
        for v, delta in pairs:
            if v in seen_t or _OPT_ADDON_RE.search(v):
                continue
            seen_t.add(v)
    ...
    for ax in axes:
        ...
        for c in combos:
            for p in ax:
                m = dict(c)
                pd = m.pop("_delta", 0) + p.get("_delta", 0)
                m.update({k: x for k, x in p.items() if k != "_delta"})
                if pd:
                    m["_delta"] = pd
                merged.append(m)
        combos = merged[:cap]
...
# 2026-07-03 패턴 감사: '+N원'(가격병기)·'★'(프로모마커)는 정당 변형에 흔해(flora 98.6%)
# 통드롭 금지 — 가격은 _split_delta 로 분리, ★는 _clean_opt 가 마커만 제거.
_OPT_ADDON_RE = re.compile(
    r"추가|사은품|증정|선택\s*안\s*함|구매\s*안\s*함|단품\s*구매|미포함|없음")


# 가격델타 병기 4종(패턴 보고서 §2.1) — "( 사양 / +N,NNN원 )"·"(+N,NNN[원])"·": +N원"
# _DELTA_TAIL: 중첩 괄호 내성형 "…(110x200cm) / +33,900원 )" — 부호 필수, 닫는 괄호 직전
_DELTA_TAIL = re.compile(r"/\s*([+-])\s*([\d,]{1,9})\s*원?\s*(?:\[?품절\]?)?\s*(?:/\s*)?(?=\))")
_DELTA_PAREN_SLASH = re.compile(r"\(\s*([^()]*?)\s*/\s*([+-]?)\s*([\d,]{1,9})\s*(원)?\s*\)")
_DELTA_PAREN = re.compile(r"\(\s*([+-])\s*([\d,]{1,9})\s*원?\s*\)")
_DELTA_COLON = re.compile(r"\s*[:：]\s*([+-]?)\s*([\d,]{1,9})\s*원")


def _split_delta(o):
    """옵션값의 가격델타 병기 분리 → (텍스트, 델타원|None).

    괄호 치수 "(1200)" 오탐 방지: 슬래시형은 '원' 또는 부호, 단독 괄호형은 부호 필수."""
    m = _DELTA_TAIL.search(o)
    if m:
        d = int(m.group(2).replace(",", "")) * (-1 if m.group(1) == "-" else 1)
        return _WS.sub(" ", o[:m.start()] + " " + o[m.end():]).strip(), d
    m = _DELTA_PAREN_SLASH.search(o)
    if m and (m.group(4) or m.group(2)):
        d = int(m.group(3).replace(",", "")) * (-1 if m.group(2) == "-" else 1)
        return _WS.sub(" ", o[:m.start()] + " " + m.group(1) + " " + o[m.end():]).strip(), d
    m = _DELTA_PAREN.search(o)
    if m:
        d = int(m.group(2).replace(",", "")) * (-1 if m.group(1) == "-" else 1)
        return _WS.sub(" ", o[:m.start()] + " " + o[m.end():]).strip(), d
    m = _DELTA_COLON.search(o)
    if m:
        d = int(m.group(2).replace(",", "")) * (-1 if m.group(1) == "-" else 1)
        return _WS.sub(" ", o[:m.start()] + " " + o[m.end():]).strip(), d
    return o, None
...
def _clean_opt(o):
    o = re.sub(r"[★☆]+", " ", o)                             # 프로모 마커(값은 정당 변형)
    o = re.sub(r"\[?\s*품절\s*\]?", " ", o)
    o = _split_delta(o)[0]                                    # 가격병기 제거(델타는 군 파서가 캡처)
    o = _OPT_ENUM_RE.sub("", o)                              # "1. "/"A1. "/"A. " 열거자 prefix
    o = re.sub(r"_\d{5,}\s*$", "", o)                        # "_100370" 내부코드
    o = re.sub(r"\s+DF\d{4,}(?:-\d+)?\b", " ", o)            # 동서 내부코드(DF925364, DF907014-1)
...
def model_options(raw_options):
    # 가격델타 보유 값은 모음전 증거에서 제외 — 델타는 "기준가 대비 차액" = 변형 신호.
    # (변형 팬아웃·옵션군 경로가 델타 값을 처리. 2026-07-03: flora 구성옵션 과분해 +5.2K 키 회귀 방지)
    raws = [o for o in (raw_options or "").split("|") if o.strip()]
    opts = [_clean_opt(o) for o in raws if _split_delta(o)[1] is None]
...
_VARIANTISH_RE = re.compile(r"(?:SS|EK|LK|KK|[SQKD])(?![가-힣A-Za-z])|\d+\s*(?:인|[wW]|구|mm|cm|개입)"
                            r"|주광색|전구색|주백색|하얀불|노란불|아이보리불|\d{4}[kK]"  # 색온도(동의어 포함)
...
def _pure_axis_groups(grp_json):
    """모음전 합성행(_opt_src)에 승계할 '순수 SKU 변형축' 군만 남김.

    승계 기준: 군의 모든 값이 color/size/firm/watt/cct 로 완전 해석(option 잔여·usage/form 없음).
    모델열거·부모선택자(종류) 군을 승계하면 이중 전개가 되므로 제외.
    (패턴 감사 2026-07-03: 합성행이 옵션군 전체를 폐기해 W/색상 군까지 소실 — 103 PDP)"""
    grp_json = (grp_json or "").strip()
    if not grp_json or grp_json == "[]":
        return ""
    try:
    ...
    for g in groups:
        label = (g.get("label") or "").strip()
        if label == "_cascade" or _ADDON_GROUP_RE.search(label):
            continue
        ...
        for v in vals:
            po = _promote_option(v)
            if po["option"] or po["usage"] or po["form"] \
                    or not any(po[a] for a in ("color", "size", "firm", "watt", "cct")):
                pure = False
                break
        if pure:
            kept.append(g)
...
def run_decompose(in_path=IN_JSONL, out_path=DECOMP_OUT):
    ...
    for r in recs:
        ...
        rows.append({
            ...
            "option_groups": (_pure_axis_groups(r.get("raw_option_groups"))
                              if "_opt_src" in r else (r.get("raw_option_groups") or "")),
    ...
    for key, rs in groups.items():
        ...
        for r in rs:
            ...
            if len(combos) >= 2:
                for cv in combos:
                    cv = dict(cv)
                    delta = cv.pop("_delta", 0)   # 변형가 = 상품가 + Σ군델타
                    va_v = dict(va)
                    va_v.update(cv)
                    if va_v.get("color"):
